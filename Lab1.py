...
# Абстрактний клас для персонажів
class Character(ABC):
    ...
    def take_damage(self, damage):
        # Розрахунок реального пошкодження з урахуванням параметра захисту
        actual_damage = max(damage - self.defense, 0)
        self.hp -= actual_damage
        self.hp = max(self.hp, 0)  # Уникаємо значення нижче нуля
...
# Конкретні реалізації класів для кожної раси
class Elf(Character):
    def special_ability(self):
